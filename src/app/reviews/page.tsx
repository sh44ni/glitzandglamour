...
type Review = {
    ...
    rating: number;
    text: string;
    source: string;        // "website" | "setmore" | "google"
...
function BadgeChip({ badge, source }: { badge?: string | null; source: string }) {
    ...
    if (source === 'google') {
        return (
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '3px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '50px', padding: '2px 8px', fontFamily: 'Poppins, sans-serif', fontSize: '10px', fontWeight: 600, color: '#ccc' }}>
        ...
        <div style={{ minHeight: '100vh', padding: '48px 20px 120px', maxWidth: '900px', margin: '0 auto', position: 'relative', zIndex: 1 }}>
            ...
            {googleReviews.length > 0 && (
                <div style={{ marginBottom: '32px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '12px' }}>
                        {googleReviews.map(r => {
                            const displayName = r.user?.name || r.authorName || 'Google User';
                            return (
                                <div key={r.id} className="glass-card" style={{ padding: '20px', borderRadius: '16px' }}>
                                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', marginBottom: '10px' }}>
                                        <div style={{ width: '40px', height: '40px', borderRadius: '50%', flexShrink: 0, overflow: 'hidden', background: '#333', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                                            {r.user?.image ? (
                                                <Image src={r.user.image} alt={displayName} fill style={{ objectFit: 'cover' }} />
                                            ) : (
                                                <span style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 700, color: '#fff', fontSize: '16px' }}>{displayName.charAt(0)}</span>
                                    ...
                                    <p style={{ fontFamily: 'Poppins, sans-serif', color: '#eee', fontSize: '13px', lineHeight: 1.7 }}>
                                        &ldquo;{r.text}&rdquo;
                                    </p>
                                    <p style={{ fontFamily: 'Poppins, sans-serif', color: '#555', fontSize: '11px', marginTop: '10px' }}>
                                        {timeAgo(r.createdAt)}
                                    </p>
                                </div>
                            );
